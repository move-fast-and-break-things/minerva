...
class CalendarTool:
    def __init__(self, calendar_url: str):
        ...
        self.refetch_interval = int(os.getenv("CALENDAR_REFETCH_INTERVAL", 15))  # Fetch interval from env, default 15 minutes
        asyncio.create_task(self._refetch_calendar_periodically())  # Start background task to refetch

    async def _fetch_calendar(self):
        """Fetch the calendar from the provided URL."""
        try:
            calendar_request = httpx.get(self.calendar_url)
